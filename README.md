...
### Redoc and Swagger
sysl-catalog comes with a sysl to swagger exporter which is autogenerated in server mode. There is also a redoc endpoint for human readability of the api spec. Both are accessable at these endpoints:
```
host:port/swagger (swagger in json format)
host:port/redoc
```

To enable the exporter and redoc endpoint, the application name must be provided and the sysl file should contain well defined rest endpoints:
```
sysl-catalog --serve <input.sysl> --appname <ApplicationName>
```
